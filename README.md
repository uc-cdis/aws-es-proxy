...
helm repo update
helm upgrade --install gen3/aws-es-proxy
```
These commands will add the Gen3 helm chart repo and install the aws-es-proxy service to your Kubernetes cluster. 
...
```bash
helm upgrade --install gen3/aws-es-proxy -f values.yaml
```
